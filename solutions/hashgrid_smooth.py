...
class HashGridEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        for l in range(self.n_levels):
            ...
            ix0 = ix0.clamp(0, N - 1)
            iy0 = iy0.clamp(0, N - 1)
            # smoothstep (C1) interpolation instead of plain bilinear (C0); removes the
            # derivative discontinuities at cell edges (Instant-NGP appendix).
            fx = xf[:, 0:1]
            fy = xf[:, 1:2]
            fx = fx * fx * (3 - 2 * fx)
            fy = fy * fy * (3 - 2 * fy)
