...
class HashGridSolution(TorchSolution):
    name = "hashgrid_pow2"
    description = ("champion pool4 config with sharper stochastic mining: sample hard points "
                   "proportional to squared error instead of raw absolute error")
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        while not ctx.expired():
            ...
            with torch.no_grad(), ac:
                ...
                mine_w = perr * perr
            hard_idx = torch.multinomial(mine_w, n_hard, replacement=False)
