...
_PRIMES_B = (2_165_219_737, 1_434_869_437)   # second, independent hash for collision decorrelation
...
class HashGridEncoding(nn.Module):
    def __init__(self, n_levels=16, n_features=2, log2_T=20,
        ...
        self.tables_b = nn.ParameterList()   # second table per HASHED level (None-slot if direct)
        self.sizes = []
        ...
        for l in range(n_levels):
            ...
            self.tables.append(nn.Parameter(torch.empty(size, n_features).uniform_(-1e-4, 1e-4)))
            # only hashed levels collide -> give them a 2nd independent table to average
            self.tables_b.append(nn.Parameter(torch.empty(size, n_features).uniform_(-1e-4, 1e-4))
                                 if not direct else nn.Parameter(torch.zeros(1, n_features)))
        self.out_dim = n_levels * n_features

    def _idx(self, ix, iy, size, primes):
        h = (ix * primes[0]) ^ (iy * primes[1])
        return h % size

    def _corner(self, l, ix, iy, size, N, direct):
        if direct:
            return self.tables[l][(iy * N + ix) % size]
        # average two independent hashes -> collisions decorrelate (Bloom-style denoising)
        a = self.tables[l][self._idx(ix, iy, size, _PRIMES)]
        b = self.tables_b[l][self._idx(ix, iy, size, _PRIMES_B)]
        return 0.5 * (a + b)
    ...
    def forward(self, x):
        ...
        for l in range(self.n_levels):
            ...
            fx = xf[:, 0:1]
            fy = xf[:, 1:2]
            c00 = self._corner(l, ix0, iy0, size, N, direct)
            c10 = self._corner(l, ix1, iy0, size, N, direct)
            c01 = self._corner(l, ix0, iy1, size, N, direct)
            c11 = self._corner(l, ix1, iy1, size, N, direct)
            c0 = c00 * (1 - fx) + c10 * fx
            c1 = c01 * (1 - fx) + c11 * fx
...
class HashGridSolution(TorchSolution):
    name = "hashgrid_dualhash"
    description = ("champion grid + DUAL HASH (2 tables/hashed level, averaged) to decorrelate collisions; + warmup over the "
                   "first 8% then cosine decay — stabilize the aggressive table LR early")
