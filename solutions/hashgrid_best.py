...
class HashGridSolution(TorchSolution):
    ...
    description = ("hash grid (24 lvl, F=2, T=2^21, Nmax=8192) + MLP 256x4, "
                   "error-weighted adaptive sampling (3x pool, 50% mining) + time-based cosine LR")
    ...
    def _set_lr(self, opt, frac):
        # cosine decay over the ACTUAL time budget (step count is unknown up front, and the
        # prior fixed-T_max schedule barely decayed in ~2000 steps -> noisy late training)
        import math
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        self.model.train()
        batch = 262_144
        pool_mult = 3          # smaller pool than hashgrid_adaptive: cheaper GT, more steps
        n_hard = batch // 2    # half error-weighted, half uniform for coverage
