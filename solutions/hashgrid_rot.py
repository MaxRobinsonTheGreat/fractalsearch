...
_R45 = 0.7071067811865476   # cos/sin(45 deg)
...
class HashGridNet(nn.Module):
    def __init__(self, n_levels=16, n_features=2, log2_T=20,
        ...
        super().__init__()
        # axis-aligned grid + a 45-deg-rotated grid (captures diagonal boundary structure
        # that axis-aligned bilinear cells handle poorly)
        self.enc = HashGridEncoding(n_levels, n_features, log2_T, n_min, n_max)
        self.enc_rot = HashGridEncoding(n_levels, n_features, log2_T, n_min, n_max)
        in_dim = self.enc.out_dim + self.enc_rot.out_dim
        dims = [in_dim] + [hidden] * (mlp_layers - 1) + [1]
        blocks = []
    ...
    def _rot(self, xy):
        # rotate 45 deg about center (0.5,0.5), rescale by cos45 so it stays in [0,1]
        cx = xy[:, 0:1] - 0.5
        cy = xy[:, 1:2] - 0.5
        rx = (cx * _R45 - cy * _R45) * _R45 + 0.5
        ry = (cx * _R45 + cy * _R45) * _R45 + 0.5
        return torch.cat([rx, ry], dim=1).clamp(0.0, 1.0)

    def forward(self, x):
        xn = self._norm(x)
        e = torch.cat([self.enc(xn), self.enc_rot(self._rot(xn))], dim=1)
        return (torch.tanh(self.net(e)) + 1) / 2


class HashGridSolution(TorchSolution):
    name = "hashgrid_rot"
    description = ("champion grid + a second 45-deg ROTATED hash grid (dual coordinate frame) "
                   "to test whether directional/axis-aligned aliasing limits the boundary fit")
    ...
    def fit(self, ctx: FitContext) -> None:
        opt = torch.optim.Adam([
            {"params": list(self.model.enc.parameters()) + list(self.model.enc_rot.parameters()),
             "peak_lr": self.TBL_LR0, "lr": self.TBL_LR0},
