...
class HashGridSolution(TorchSolution):
    name = "hashgrid_ema"
    description = ("hash grid (24 lvl, F=2, T=2^21, Nmax=8192) + MLP 256x4, adaptive mining + "
                   "time-LR, EMA weights (decay 0.997) evaluated at the end to denoise the tail")
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        d = self.EMA_DECAY
        # EMA shadow of every parameter — averages the converged, low-LR tail to kill the
        # SGD oscillation that hurts high-frequency fits. Cost: one lerp per param per step.
        ema = [p.detach().clone() for p in self.model.parameters()]
        step = 0
