...
class HashGridSolution(TorchSolution):
    name = "hashgrid_2phase"
    description = ("champion (T=2^24, batch 524k, table LR 6e-1, Nmax=32768, batch 768k, 2-phase: cheap uniform 65% then heavy mining 35%) + warmup over the "
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        step = 0
        # Two-phase: PHASE 1 (first 65%) cheap fresh uniform, no mining -> many fast steps fit
        # the easy bulk. PHASE 2 (last 35%) expensive heavy mining -> refine the HF boundary.
        PHASE1 = 0.65
        ...
        while not ctx.expired():
            ...
            if frac < PHASE1:
                coords, targets = ctx.sample(batch)          # cheap: GT on batch only
            else:
                with torch.no_grad(), ac:
                    pcoords, ptargets = ctx.sample(batch * pool_mult)
                    perr = (self.model(pcoords).reshape(-1).float() - ptargets).abs() + 1e-6
            ...
            step += 1
            if step % 200 == 0:
                ctx.log(f"step {step} loss {loss.item():.5f} ph{1 if frac<PHASE1 else 2} (left {ctx.time_left():.0f}s)")
