...
class HashGridNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(self.net(e))  # gentler-slope output head vs (tanh+1)/2


class HashGridSolution(TorchSolution):
    name = "hashgrid_sigmoid"
    description = ("champion (T=2^24, batch 524k, table LR 6e-1, Nmax=32768, batch 768k, 4x pool, 90% hard, sigmoid head) + warmup over the "
