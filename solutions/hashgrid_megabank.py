...
class MegaBankSolution(TorchSolution):
    name = "hashgrid_megabank"
    description = ("triton champion + 250M-point precomputed GT bank: pool GT was 81% "
                   "of step time; pay it once (~30s) for ~4x more steps. 250M = 27x "
                   "eval-grid density so memorization should be invisible at eval res "
                   "(unlike the old 32M bank).")
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        # Precompute the GT bank once (chunked to bound peak memory).
        bank_c = torch.empty(self.BANK_N, 2, device=ctx.device)
        bank_t = torch.empty(self.BANK_N, device=ctx.device)
        for i in range(0, self.BANK_N, self.BANK_CHUNK):
            n = min(self.BANK_CHUNK, self.BANK_N - i)
            c, t = ctx.sample(n)
            bank_c[i:i + n] = c
            bank_t[i:i + n] = t
        ctx.log(f"bank of {self.BANK_N/1e6:.0f}M points ready at t={ctx.elapsed():.0f}s")

        while not ctx.expired():
            self._set_lr(opt, min(1.0, ctx.elapsed() / budget))
            bidx = torch.randint(0, self.BANK_N, (batch * pool_mult,), device=ctx.device)
            pcoords, ptargets = bank_c[bidx], bank_t[bidx]
