...
class HashGridSolution(TorchSolution):
    name = "hashgrid_h64"
    description = ("champion config with smaller decoder MLP width 64 instead of 256 "
                   "to test whether the hash grid can carry detail with a very small decoder")
    ...
    def __init__(self):
        self.model = HashGridNet(n_levels=12, n_features=2, log2_T=24,
                                 n_min=16, n_max=32768, hidden=64, mlp_layers=4).to(self.device)
