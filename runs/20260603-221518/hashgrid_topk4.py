...
class HashGridSolution(TorchSolution):
    name = "hashgrid_topk4"
    description = ("champion pool4 config, but select true top-k highest-error pool points "
                   "instead of error-proportional multinomial mining")
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        while not ctx.expired():
            ...
            hard_idx = torch.topk(perr, n_hard, sorted=False).indices
