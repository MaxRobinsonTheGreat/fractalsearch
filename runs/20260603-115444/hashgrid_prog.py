...
class HashGridEncoding(nn.Module):
    def __init__(self, n_levels=16, n_features=2, log2_T=20,
        ...
        # per-level gate for progressive (coarse-to-fine) unlocking; 1.0 = fully active
        self.register_buffer("gate", torch.ones(n_levels))
    ...
    def forward(self, x):
        ...
        for l in range(self.n_levels):
            ...
            c0 = c00 * (1 - fx) + c10 * fx
            c1 = c01 * (1 - fx) + c11 * fx
            feats.append((c0 * (1 - fy) + c1 * fy) * self.gate[l])
...
class HashGridSolution(TorchSolution):
    name = "hashgrid_prog"
    description = ("hash grid (24 lvl, F=2, T=2^23, Nmax=8192) + MLP 256x4, adaptive mining + "
                   "time-LR + progressive coarse-to-fine level unlocking (BARF-style windowing)")

    LR0 = 1e-2
    LR_MIN = 1e-4
    UNLOCK_FRAC = 0.6   # all levels fully active by this fraction of the budget
    ...
    def _set_gate(self, frac):
        # smooth coarse-to-fine: level l fully on once the "frontier" alpha = (frac/UNLOCK)*L
        # passes l; partial cosine ramp over a 1-level-wide window for each level.
        import math
        L = self.model.enc.n_levels
        alpha = min(1.0, frac / self.UNLOCK_FRAC) * L
        gate = self.model.enc.gate
        for l in range(L):
            w = alpha - l
            if w <= 0:
                gate[l] = 0.0
                ...
                gate[l] = 0.5 * (1 - math.cos(math.pi * w))
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        while not ctx.expired():
            ...
            step += 1
            if step % 200 == 0:
                ctx.log(f"step {step} loss {loss.item():.5f} gate[-1]={self.model.enc.gate[-1]:.2f} "
                        f"(left {ctx.time_left():.0f}s)")
        # ensure all levels active for evaluation
        self.model.enc.gate.fill_(1.0)
