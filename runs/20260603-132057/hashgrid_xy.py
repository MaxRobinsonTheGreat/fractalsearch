...
class HashGridNet(nn.Module):
    def __init__(self, n_levels=16, n_features=2, log2_T=20,
        ...
        # concat raw normalized coord to the encoding: helps the MLP resolve hash collisions
        # and supplies a smooth absolute-position signal.
        dims = [self.enc.out_dim + 2] + [hidden] * (mlp_layers - 1) + [1]
        blocks = []
    ...
    def forward(self, x):
        xn = self._norm(x)
        e = torch.cat([self.enc(xn), xn], dim=1)
...
class HashGridSolution(TorchSolution):
    ...
    description = ("hash grid (24 lvl, F=2, T=2^23, Nmax=8192) + MLP 256x4, adaptive mining + "
                   "16 levels + raw coord concatenated to grid features")
    ...
    def __init__(self):
        self.model = HashGridNet(n_levels=16, n_features=2, log2_T=23,
