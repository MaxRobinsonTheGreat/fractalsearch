...
class HashGridSolution(TorchSolution):
    name = "hashgrid_bigT_mine"
    description = ("hash grid (24 lvl, F=2, T=2^21, Nmax=8192) + MLP 256x4, "
                   "adaptive mining + time-LR, T=2^22 + heavy mining (train fine entries via boundary concentration)")

    LR0 = 1e-2
    LR_MIN = 1e-4

    def __init__(self):
        self.model = HashGridNet(n_levels=24, n_features=2, log2_T=22,
