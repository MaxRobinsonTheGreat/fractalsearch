...
class SineLayer(nn.Module):
    """SIREN layer (Sitzmann et al. 2020): y = sin(omega0 * (W x + b)), with the SIREN
    init that keeps sinusoidal activations well-conditioned."""
    def __init__(self, in_f, out_f, omega0=30.0, first=False):
        super().__init__()
        self.omega0 = omega0
...
class HashGridNet(nn.Module):
    def __init__(self, n_levels=16, n_features=2, log2_T=20,
                 n_min=16, n_max=4096, hidden=128, mlp_layers=3, omega0=30.0):
        super().__init__()
        self.enc = HashGridEncoding(n_levels, n_features, log2_T, n_min, n_max)
        # SIREN decoder: sine activations naturally produce the target's periodic oscillation.
...
class HashGridSolution(TorchSolution):
    name = "hashgrid_siren"
    description = ("hash grid (24 lvl, F=2, T=2^23, Nmax=8192) + SIREN sine-activated decoder "
                   "256x4 (omega0=30), adaptive mining + split LRs (table 5e-2 / MLP 5e-3)")
