...
class HashGridSolution(TorchSolution):
    name = "hashgrid_wloss"
    description = ("champion grid (T=2^24, Nmax=32768, batch 768k) + ERROR-WEIGHTED LOSS instead "
                   "of a mining pool: GT only on trained points (3x less GT -> more steps)")
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        self.model.train()
        # No mining POOL: GT only on the points we train on (3x less GT -> ~3x more steps).
        # Emphasize the boundary via an error-WEIGHTED loss (moderate, to avoid the densify
        # over-focus failure): w ∝ |err| + 0.25*mean|err|, normalized to mean 1, clamped.
        batch = 786_432
        ...
        while not ctx.expired():
            ...
            with ac:
                ...
                err = pred.float() - targets
                with torch.no_grad():
                    a = err.abs()
                    w = a + 0.25 * a.mean()
                    w = (w / w.mean()).clamp(max=8.0)
                loss = torch.mean(w * err * err)
