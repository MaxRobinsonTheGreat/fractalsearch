...
class HashGridSolution(TorchSolution):
    name = "hashgrid_compile"
    description = ("hashgrid_lr config + torch.compile to fuse the memory-bound gather/interp/MLP "
                   "kernels for higher step throughput (fixed shapes -> compiles once)")
    ...
    def __init__(self):
        ...
        self.cmodel = torch.compile(self.model)  # compiled view; shares params with self.model
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        while not ctx.expired():
            ...
            with torch.no_grad(), ac:
                ...
                ppred = self.cmodel(pcoords).reshape(-1)
            ...
            with ac:
                pred = self.cmodel(coords).reshape(-1)
