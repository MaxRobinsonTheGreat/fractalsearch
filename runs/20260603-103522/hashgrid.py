...
class HashGridSolution(TorchSolution):
    name = "hashgrid"
    description = "Instant-NGP multiresolution hash grid (16 lvl, F=2, T=2^20, Nmax=4096) + GELU MLP 128x3"

    def __init__(self):
        self.model = HashGridNet(n_levels=16, n_features=2, log2_T=20,
                                 n_min=16, n_max=4096, hidden=128, mlp_layers=3).to(self.device)

    def fit(self, ctx: FitContext) -> None:
        # higher LR for hash tables, standard for MLP (per Instant-NGP)
