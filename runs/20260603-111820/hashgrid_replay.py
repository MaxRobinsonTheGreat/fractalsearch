...
class HashGridSolution(TorchSolution):
    name = "hashgrid_replay"
    description = ("hash grid (24 lvl, F=2, T=2^21, Nmax=8192) + MLP 256x4, "
                   "8M churning replay bank (cheap GT) + free hard-mining from stored targets + time-LR")

    LR0 = 1e-2
    LR_MIN = 1e-4
    BANK = 8_000_000        # replay buffer (churns -> quasi-fresh, no fixed-bank overfit)
    REFRESH = 131_072       # fresh GT points per step (the ONLY per-step ground-truth cost)
    ...
    def _set_lr(self, opt, frac):
        import math
        lr = self.LR_MIN + 0.5 * (self.LR0 - self.LR_MIN) * (1 + math.cos(math.pi * frac))
        for g in opt.param_groups:
            g["lr"] = lr

    def fit(self, ctx: FitContext) -> None:
        # fill the replay bank once
        cs, ts = [], []
        done = 0
        while done < self.BANK:
            n = min(2_000_000, self.BANK - done)
            c, t = ctx.sample(n)
            cs.append(c); ts.append(t); done += n
        coords_bank = torch.cat(cs)
        targets_bank = torch.cat(ts)
        dev = coords_bank.device
        ctx.log(f"bank filled {self.BANK} pts at {ctx.elapsed():.1f}s")

        opt = torch.optim.Adam(self.model.parameters(), lr=self.LR0, betas=(0.9, 0.99), eps=1e-15)
        self.model.train()
        batch = 262_144
        pool_mult = 3          # mining pool drawn from the BANK (no GT cost — targets stored)
        n_hard = batch // 2
        ...
        while not ctx.expired():
            ...
            # churn the bank with a small fresh slice (the only per-step ground-truth call)
            c, t = ctx.sample(self.REFRESH)
            end = ptr + self.REFRESH
            if end <= self.BANK:
                coords_bank[ptr:end] = c; targets_bank[ptr:end] = t
            else:
                k = self.BANK - ptr
                coords_bank[ptr:] = c[:k]; targets_bank[ptr:] = t[:k]
                coords_bank[:end - self.BANK] = c[k:]; targets_bank[:end - self.BANK] = t[k:]
            ptr = end % self.BANK

            # hard-mine from the bank using model error only (stored targets, no GT recompute)
            with torch.no_grad():
                pool_idx = torch.randint(0, self.BANK, (batch * pool_mult,), device=dev)
                pc = coords_bank[pool_idx]; pt = targets_bank[pool_idx]
                perr = (self.model(pc).reshape(-1) - pt).abs() + 1e-6
