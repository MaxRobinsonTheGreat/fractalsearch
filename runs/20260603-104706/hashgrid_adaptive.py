...
class HashGridSolution(TorchSolution):
    name = "hashgrid_adaptive"
    description = ("hash grid (24 lvl, F=2, T=2^21, Nmax=8192) + MLP 256x4, "
                   "error-weighted adaptive sampling (4x pool, 50% hard-mining)")
    ...
    def fit(self, ctx: FitContext) -> None:
        ...
        self.model.train()
        batch = 262_144
        pool_mult = 4          # sample this many x batch, mine the hardest
        n_hard = batch // 2    # half the batch from error-weighted mining, half uniform
        step = 0
        while not ctx.expired():
            # draw a big uniform pool, score it cheaply (no grad), keep the hard tail
            with torch.no_grad():
                pcoords, ptargets = ctx.sample(batch * pool_mult)
                ppred = self.model(pcoords).reshape(-1)
                perr = (ppred - ptargets).abs() + 1e-6
            # error-weighted (hard) subset + a uniform subset for coverage/exploration
